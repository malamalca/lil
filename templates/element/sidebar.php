...
if ($this->getRequest()->is('mobile')) {
    $popup_headerMenu = [];
    foreach ($admin_sidebar as $panel) {
        if (!empty($panel['active']) && !empty($panel['items'])) {
            foreach ($panel['items'] as $li) {
                if (!empty($li['visible'])) {
        ...
        if (!empty($panel['active']) && !empty($panel['items']) && !empty($panel['visible'])) {
            ...
            foreach ($panel['items'] as $li) {
                if (!empty($li['visible'])) {
                    ...
                    if (
                        ((!isset($li['expand']) || is_null($li['expand'])) && !empty($li['submenu'])) ||
                        (!empty($li['expand']))
                    ) {
                        $classes[] = 'expanded';
                    }
                    ...
                    // submenu
                    if (!empty($li['submenu'])) {
                        printf('<ul%s>' . PHP_EOL, !isset($li['expand']) || is_null($li['expand']) ?
                            '' :
                            (empty($li['expand']) ? ' class="folded"' : ' class="expanded"'));
                        foreach ($li['submenu'] as $lis) {
                            if (!empty($lis['visible'])) {
...
if ($this->getRequest()->is('mobile')) {
    ...
    $this->Lil->jsReady('function adjustSidebar() { ' .
        '$("ul.sidebar-menu").height($("#sidebar").height() - $("ul#sidebar-panels").height() - 15); }');
    $this->Lil->jsReady('$(window).resize(function() { adjustSidebar(); });');
    $this->Lil->jsReady('adjustSidebar();');
    $this->Lil->jsReady('$("span.expand-toggle").click(function(){ ' .
        '$(this).next("ul").toggle().closest("li").toggleClass("expanded");});');
    $this->Lil->jsReady('$("ul.sidebar-menu").bind("mousewheel", function (e, delta) { ' .
        '$(this).scrollTop($(this).scrollTop()- delta * 8); event.preventDefault(); });');
}
